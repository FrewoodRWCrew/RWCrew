// The "Akties" tab: the list of intervention requests. By default it shows
...
// box filters across the main text fields. Tap a request to view/edit it;
...
import { formatDateTime, teamLabel } from "../../../../components/module-3/format";
...
export default function ActionsScreen() {
  ...
  // Apply the status filter, then the search text.
        ...
        <Card style={styles.item}>
          ...
          {(item.preferred_delivery_at || item.delivery_location) && (
            <Text style={[styles.small, { color: colors.muted }]} numberOfLines={1}>
              {[formatDateTime(item.preferred_delivery_at, language), item.delivery_location].filter(Boolean).join(" · ")}
            </Text>
          )}
        </Card>
      </Pressable>
