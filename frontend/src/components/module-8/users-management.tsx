...
// Altsien Select, a dropdown to change their role, and a "New
// user" action that can create a brand-new account, scoped to
// Altsien Select only. Direct mirror of MasterData's
// users-management.tsx (see docs/module-custom-roles-pattern.md).

import { useState } from "react";
import { useTranslations } from "next-intl";
import { toast } from "sonner";
import { ApiError, createOrGrantAltsienSelectUser, setAltsienSelectUserRole } from "@/lib/api";
...
interface UsersManagementProps {
  ...
  initialUsers: AltsienSelectUserSummary[];
}
...
export function UsersManagement({ roles, initialUsers }: UsersManagementProps) {
  const t = useTranslations("altsienSelect.users");
  ...
  function upsertUser(updatedUser: AltsienSelectUserSummary) {
    setUsers((current) => {
  ...
  async function handleChangeUserRole(user: AltsienSelectUserSummary, roleIdValue: string) {
    const roleId = roleIdValue === NO_ROLE_VALUE ? null : Number(roleIdValue);
    try {
      const updatedUser = await setAltsienSelectUserRole(user.user_id, roleId);
...
interface CreateOrGrantUserDialogProps {
  roles: AltsienSelectRole[];
  onSaved: (user: AltsienSelectUserSummary) => void;
}
...
function CreateOrGrantUserDialog({ roles, onSaved }: CreateOrGrantUserDialogProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      const savedUser = await createOrGrantAltsienSelectUser({
        email,
      ...
      <DialogContent>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <Label htmlFor="altsienselect-new-user-email">{t("emailLabel")}</Label>
            <Input
              id="altsienselect-new-user-email"
              type="email"
              value={email}
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="altsienselect-new-user-name">{t("newUserNameLabel")}</Label>
            <Input
              id="altsienselect-new-user-name"
              value={displayName}
              onChange={(event) => setDisplayName(event.target.value)}
            />
          </div>
          <div className="flex flex-col gap-2">
            <Label htmlFor="altsienselect-new-user-password">{t("newUserPasswordLabel")}</Label>
            <Input
              id="altsienselect-new-user-password"
              type="password"
              value={password}
