...
// of statuses a special request can have (seeded New / In Progress /
// Completed). A copy of Intervention Requests' status screen
// (components/module-3/intervention-status-management.tsx) with one extra
// field, the sort order: the lowest one is the status every new request
// starts in, and the Kernlid can only change a request while it's there.
...
import {
  ApiError,
  createAltsienSelectRequestStatus,
  deleteAltsienSelectRequestStatus,
  updateAltsienSelectRequestStatus,
} from "@/lib/api";
import type {
  AltsienSelectRequestStatus as RequestStatus,
  AltsienSelectRequestStatusInput as RequestStatusInput,
} from "@/lib/types";
...
interface RequestStatusManagementProps {
  initialStatuses: RequestStatus[];
}

const EMPTY_FORM: RequestStatusInput = { name: "", is_open: true, color: "gray", sort_order: 0 };

function toFormValues(item: RequestStatus): RequestStatusInput {
  return { name: item.name, is_open: item.is_open, color: item.color, sort_order: item.sort_order };
}
...
export function RequestStatusManagement({ initialStatuses }: RequestStatusManagementProps) {
  const t = useTranslations("altsienSelect.status");
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="rounded-md border [&>div]:max-h-[65vh] [&>div]:overflow-y-auto">
        <Table>
          <TableHeader>
            <TableRow>
              <TableHead className="sticky top-0 z-20 bg-background font-bold underline">{t("columnOrder")}</TableHead>
...
interface StatusFormDialogProps {
  ...
  onSaved: (item: RequestStatus) => void;
}
...
function StatusFormDialog({ item, trigger, onSaved }: StatusFormDialogProps) {
  ...
  const [form, setForm] = useState<RequestStatusInput>(item ? toFormValues(item) : EMPTY_FORM);
  ...
  function updateField<K extends keyof RequestStatusInput>(field: K, value: RequestStatusInput[K]) {
  ...
  async function handleSubmit() {
    ...
    try {
      const payload: RequestStatusInput = { ...form, name: form.name.trim() };
      const savedItem = isEditing
        ? await updateAltsienSelectRequestStatus(item.id, payload)
        : await createAltsienSelectRequestStatus(payload);
    ...
    <Dialog open={isOpen} onOpenChange={handleOpenChange}>
      ...
      <DialogContent>
        ...
        <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="status-sort-order">{t("orderLabel")}</Label>
            <Input
            ...
            />
            <p className="text-xs text-muted-foreground">{t("orderHelp")}</p>
          </div>
...
function DeleteStatusAlertDialog({ item, onDeleted }: DeleteStatusAlertDialogProps) {
  ...
  async function handleConfirmDelete() {
    setIsDeleting(true);
    try {
      await deleteAltsienSelectRequestStatus(item.id);
