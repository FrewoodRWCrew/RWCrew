...
// each role's permission matrix (view/create/edit/delete per screen). User
// management lives on its own separate screen — see users-management.tsx
// — gated independently via its own "kartracker.users" permission. Direct
// mirror of Intervention Requests'/MasterData's roles-management.tsx (see
...
import {
  ApiError,
  createKarTrackerRole,
  deleteKarTrackerRole,
  renameKarTrackerRole,
  setKarTrackerRolePermissions,
} from "@/lib/api";
import type { KarTrackerRole, KarTrackerScreen, KarTrackerScreenPermission } from "@/lib/types";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
interface RolesManagementProps {
  initialScreens: KarTrackerScreen[];
  initialRoles: KarTrackerRole[];
}

export function RolesManagement({ initialScreens, initialRoles }: RolesManagementProps) {
  const t = useTranslations("karTracker.roles");
  const [screens] = useState(initialScreens);
  const [roles, setRoles] = useState(initialRoles);

  function upsertRole(updatedRole: KarTrackerRole) {
    setRoles((current) => {
...
interface CreateRoleDialogProps {
  onCreated: (role: KarTrackerRole) => void;
}
...
function CreateRoleDialog({ onCreated }: CreateRoleDialogProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      const newRole = await createKarTrackerRole(name);
...
interface RenameRoleDialogProps {
  role: KarTrackerRole;
  onRenamed: (role: KarTrackerRole) => void;
}
...
function RenameRoleDialog({ role, onRenamed }: RenameRoleDialogProps) {
  ...
  async function handleSubmit() {
    setIsSubmitting(true);
    try {
      const updatedRole = await renameKarTrackerRole(role.id, name);
...
function DeleteRoleAlertDialog({ role, onDeleted }: DeleteRoleAlertDialogProps) {
  ...
  async function handleConfirmDelete() {
    setIsDeleting(true);
    try {
      await deleteKarTrackerRole(role.id);
...
interface EditPermissionsDialogProps {
  ...
  onSaved: (role: KarTrackerRole) => void;
}
...
function EditPermissionsDialog({ role, screens, onSaved }: EditPermissionsDialogProps) {
  ...
  const [isOpen, setIsOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [permissionsByScreenId, setPermissionsByScreenId] = useState<Map<number, KarTrackerScreenPermission>>(
    new Map(),
  );
  ...
  async function handleSubmit() {
    setIsSubmitting(true);
    try {
      const updatedRole = await setKarTrackerRolePermissions(
        role.id,
        screens.map((screen) => {
