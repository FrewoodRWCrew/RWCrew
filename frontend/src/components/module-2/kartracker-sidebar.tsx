...
// KarStatussen/Delivery Dates/Data Upload/Download/Grondplan, the Karlijst phase), and
...
export function KarTrackerSidebar({ viewableScreenKeys }: KarTrackerSidebarProps) {
  ...
  const tGroundplan = useTranslations("karTracker.groundplan");
  const tDeliveryDates = useTranslations("karTracker.deliveryDates");
  ...
  const tPlanKar = useTranslations("karTracker.planKar");
  ...
  const canViewGroundplan = viewableScreenKeys.includes("kartracker.groundplan");
  const canViewDeliveryDates = viewableScreenKeys.includes("kartracker.leverdata");
  ...
  const canViewPlanKar = viewableScreenKeys.includes("kartracker.plankar");
  ...
  return (
    <nav className="flex h-full w-60 shrink-0 flex-col gap-1 border-r bg-sidebar p-4 text-sidebar-foreground">
      ...
      {(canViewActions || canViewKarPlanning || canViewKarMap || canViewPlanKar) && (
        <>
          ...
          {canViewPlanKar && (
            <Link
              href="/modules/module-2/actions/plan-kar"
              className={linkClassName("/modules/module-2/actions/plan-kar", 1)}
            >
              {tPlanKar("title")}
            </Link>
          )}
      ...
      {(canViewKarManagement ||
        ...
        <>
          ...
          {canViewDeliveryDates && (
            <Link
              href="/modules/module-2/delivery-dates"
              className={linkClassName("/modules/module-2/delivery-dates", 1)}
            >
              {tDeliveryDates("title")}
            </Link>
          )}
          {canViewDataUploadDownload && (
            <Link
          ...
          {canViewGroundplan && (
            <Link href="/modules/module-2/groundplan" className={linkClassName("/modules/module-2/groundplan", 1)}>
              {tGroundplan("title")}
            </Link>
          )}
        </>
