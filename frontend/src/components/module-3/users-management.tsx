...
// Intervention Requests, a dropdown to change their role, and a "New
// user" action that can create a brand-new account, scoped to
// Intervention Requests only. Direct mirror of MasterData's
// users-management.tsx (see docs/module-custom-roles-pattern.md).

import { useState } from "react";
import { useTranslations } from "next-intl";
import { toast } from "sonner";
import { ApiError, createOrGrantInterventionRequestsUser, setInterventionRequestsUserRole } from "@/lib/api";
...
interface UsersManagementProps {
  ...
  initialUsers: InterventionRequestsUserSummary[];
}
...
export function UsersManagement({ roles, initialUsers }: UsersManagementProps) {
  const t = useTranslations("interventionRequests.users");
  ...
  function upsertUser(updatedUser: InterventionRequestsUserSummary) {
    setUsers((current) => {
  ...
  async function handleChangeUserRole(user: InterventionRequestsUserSummary, roleIdValue: string) {
    const roleId = roleIdValue === NO_ROLE_VALUE ? null : Number(roleIdValue);
    try {
      const updatedUser = await setInterventionRequestsUserRole(user.user_id, roleId);
...
interface CreateOrGrantUserDialogProps {
  roles: InterventionRequestsRole[];
  onSaved: (user: InterventionRequestsUserSummary) => void;
}
...
function CreateOrGrantUserDialog({ roles, onSaved }: CreateOrGrantUserDialogProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      const savedUser = await createOrGrantInterventionRequestsUser({
        email,
      ...
      <DialogContent>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <Label htmlFor="interventionrequests-new-user-email">{t("emailLabel")}</Label>
            <Input
              id="interventionrequests-new-user-email"
              type="email"
              value={email}
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="interventionrequests-new-user-name">{t("newUserNameLabel")}</Label>
            <Input
              id="interventionrequests-new-user-name"
              value={displayName}
              onChange={(event) => setDisplayName(event.target.value)}
            />
          </div>
          <div className="flex flex-col gap-2">
            <Label htmlFor="interventionrequests-new-user-password">{t("newUserPasswordLabel")}</Label>
            <Input
              id="interventionrequests-new-user-password"
              type="password"
              value={password}
