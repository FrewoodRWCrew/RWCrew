...
// "interventionrequests.users" permission. Direct mirror of MasterData's
...
import {
  ApiError,
  createInterventionRequestsRole,
  deleteInterventionRequestsRole,
  renameInterventionRequestsRole,
  setInterventionRequestsRolePermissions,
} from "@/lib/api";
import type { InterventionRequestsRole, InterventionRequestsScreen, InterventionRequestsScreenPermission } from "@/lib/types";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
interface RolesManagementProps {
  initialScreens: InterventionRequestsScreen[];
  initialRoles: InterventionRequestsRole[];
}

export function RolesManagement({ initialScreens, initialRoles }: RolesManagementProps) {
  const t = useTranslations("interventionRequests.roles");
  const [screens] = useState(initialScreens);
  const [roles, setRoles] = useState(initialRoles);

  function upsertRole(updatedRole: InterventionRequestsRole) {
    setRoles((current) => {
...
interface CreateRoleDialogProps {
  onCreated: (role: InterventionRequestsRole) => void;
}
...
function CreateRoleDialog({ onCreated }: CreateRoleDialogProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      const newRole = await createInterventionRequestsRole(name);
...
interface RenameRoleDialogProps {
  role: InterventionRequestsRole;
  onRenamed: (role: InterventionRequestsRole) => void;
}
...
function RenameRoleDialog({ role, onRenamed }: RenameRoleDialogProps) {
  ...
  async function handleSubmit() {
    setIsSubmitting(true);
    try {
      const updatedRole = await renameInterventionRequestsRole(role.id, name);
...
function DeleteRoleAlertDialog({ role, onDeleted }: DeleteRoleAlertDialogProps) {
  ...
  async function handleConfirmDelete() {
    setIsDeleting(true);
    try {
      await deleteInterventionRequestsRole(role.id);
...
interface EditPermissionsDialogProps {
  ...
  onSaved: (role: InterventionRequestsRole) => void;
}
...
function EditPermissionsDialog({ role, screens, onSaved }: EditPermissionsDialogProps) {
  ...
  const [permissionsByScreenId, setPermissionsByScreenId] = useState<
    Map<number, InterventionRequestsScreenPermission>
  >(new Map());
  ...
  async function handleSubmit() {
    setIsSubmitting(true);
    try {
      const updatedRole = await setInterventionRequestsRolePermissions(
        role.id,
        screens.map((screen) => {
