...
// User management lives on its own separate screen — see
// users-management.tsx — gated independently via its own "tagscan.users"
// permission.
//
// Follows the same Dialog/AlertDialog/Table/toast patterns already used
// in access-management.tsx and season-management.tsx.

import { useState } from "react";
import { KeyRound, Pencil, Trash2 } from "lucide-react";
import { useTranslations } from "next-intl";
import { toast } from "sonner";
import { ApiError, createTagscanRole, deleteTagscanRole, renameTagscanRole, setTagscanRolePermissions } from "@/lib/api";
import type { TagscanRole, TagscanScreen, TagscanScreenPermission } from "@/lib/types";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
interface RolesManagementProps {
  initialScreens: TagscanScreen[];
  initialRoles: TagscanRole[];
}

export function RolesManagement({ initialScreens, initialRoles }: RolesManagementProps) {
  const t = useTranslations("tagscan.roles");
  const [screens] = useState(initialScreens);
  const [roles, setRoles] = useState(initialRoles);

  function upsertRole(updatedRole: TagscanRole) {
    setRoles((current) => {
...
interface CreateRoleDialogProps {
  onCreated: (role: TagscanRole) => void;
}
...
function CreateRoleDialog({ onCreated }: CreateRoleDialogProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      const newRole = await createTagscanRole(name);
...
interface RenameRoleDialogProps {
  role: TagscanRole;
  onRenamed: (role: TagscanRole) => void;
}
...
function RenameRoleDialog({ role, onRenamed }: RenameRoleDialogProps) {
  ...
  async function handleSubmit() {
    setIsSubmitting(true);
    try {
      const updatedRole = await renameTagscanRole(role.id, name);
...
function DeleteRoleAlertDialog({ role, onDeleted }: DeleteRoleAlertDialogProps) {
  ...
  async function handleConfirmDelete() {
    setIsDeleting(true);
    try {
      await deleteTagscanRole(role.id);
...
function EditPermissionsDialog({ role, screens, onSaved }: EditPermissionsDialogProps) {
  ...
  const [isOpen, setIsOpen] = useState(false);
  const [isSubmitting, setIsSubmitting] = useState(false);
  const [permissionsByScreenId, setPermissionsByScreenId] = useState<Map<number, TagscanScreenPermission>>(
    new Map(),
  );
  ...
  async function handleSubmit() {
    setIsSubmitting(true);
    try {
      const updatedRole = await setTagscanRolePermissions(
        role.id,
        screens.map((screen) => {
