...
// Tagscan's "Users" screen: everyone with access to Tagscan, a dropdown
// to change their role, and a "New user" action that can create a
// brand-new account, scoped to Tagscan only. Split out from the "Roles"
// screen so the two can be gated by independent permissions (see
// roles-management.tsx for role CRUD/permission-matrix editing).

import { useState } from "react";
import { useTranslations } from "next-intl";
import { toast } from "sonner";
import { ApiError, createOrGrantTagscanUser, setTagscanUserRole } from "@/lib/api";
...
interface UsersManagementProps {
  roles: TagscanRole[];
  initialUsers: TagscanUserSummary[];
}
...
export function UsersManagement({ roles, initialUsers }: UsersManagementProps) {
  const t = useTranslations("tagscan.users");
  ...
  function upsertUser(updatedUser: TagscanUserSummary) {
    setUsers((current) => {
  ...
  async function handleChangeUserRole(user: TagscanUserSummary, roleIdValue: string) {
    const roleId = roleIdValue === NO_ROLE_VALUE ? null : Number(roleIdValue);
    try {
      const updatedUser = await setTagscanUserRole(user.user_id, roleId);
...
interface CreateOrGrantUserDialogProps {
  roles: TagscanRole[];
  onSaved: (user: TagscanUserSummary) => void;
}
...
function CreateOrGrantUserDialog({ roles, onSaved }: CreateOrGrantUserDialogProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      const savedUser = await createOrGrantTagscanUser({
        email,
      ...
      <DialogContent>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <Label htmlFor="tagscan-new-user-email">{t("emailLabel")}</Label>
            <Input
              id="tagscan-new-user-email"
              type="email"
              value={email}
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="tagscan-new-user-name">{t("newUserNameLabel")}</Label>
            <Input
              id="tagscan-new-user-name"
              value={displayName}
              onChange={(event) => setDisplayName(event.target.value)}
            />
          </div>
          <div className="flex flex-col gap-2">
            <Label htmlFor="tagscan-new-user-password">{t("newUserPasswordLabel")}</Label>
            <Input
              id="tagscan-new-user-password"
              type="password"
              value={password}
