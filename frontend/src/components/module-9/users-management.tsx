...
// MasterData's "Users" screen: everyone with access to MasterData, a
// dropdown to change their role, and a "New user" action that can create
// a brand-new account, scoped to MasterData only. Direct mirror of
// TagScan's users-management.tsx (see docs/module-custom-roles-pattern.md).

import { useState } from "react";
import { useTranslations } from "next-intl";
import { toast } from "sonner";
import { ApiError, createOrGrantMasterDataUser, setMasterDataUserRole } from "@/lib/api";
...
interface UsersManagementProps {
  ...
  initialUsers: MasterDataUserSummary[];
}
...
export function UsersManagement({ roles, initialUsers }: UsersManagementProps) {
  const t = useTranslations("masterdata.users");
  ...
  function upsertUser(updatedUser: MasterDataUserSummary) {
    setUsers((current) => {
  ...
  async function handleChangeUserRole(user: MasterDataUserSummary, roleIdValue: string) {
    const roleId = roleIdValue === NO_ROLE_VALUE ? null : Number(roleIdValue);
    try {
      const updatedUser = await setMasterDataUserRole(user.user_id, roleId);
...
interface CreateOrGrantUserDialogProps {
  roles: MasterDataRole[];
  onSaved: (user: MasterDataUserSummary) => void;
}
...
function CreateOrGrantUserDialog({ roles, onSaved }: CreateOrGrantUserDialogProps) {
  ...
  async function handleSubmit() {
    ...
    try {
      const savedUser = await createOrGrantMasterDataUser({
        email,
      ...
      <DialogContent>
        ...
        <div className="flex flex-col gap-4">
          <div className="flex flex-col gap-2">
            <Label htmlFor="masterdata-new-user-email">{t("emailLabel")}</Label>
            <Input
              id="masterdata-new-user-email"
              type="email"
              value={email}
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="masterdata-new-user-name">{t("newUserNameLabel")}</Label>
            <Input
              id="masterdata-new-user-name"
              value={displayName}
              onChange={(event) => setDisplayName(event.target.value)}
            />
          </div>
          <div className="flex flex-col gap-2">
            <Label htmlFor="masterdata-new-user-password">{t("newUserPasswordLabel")}</Label>
            <Input
              id="masterdata-new-user-password"
              type="password"
              value={password}
