# This file defines the "Tagscan_role_permissions" table: what one
# Tagscan role is allowed to do on one Tagscan screen.
#
# Each row covers exactly one (role, screen) pair, with four separate
# yes/no switches — a role might be allowed to VIEW a screen but not
# EDIT anything on it, for example. If a role has no row at all for a
# given screen, that means "no access whatsoever" for that screen — see
# app/modules/module_1/deps.py for how this is checked.
...
class TagscanRolePermission(Base):
    """One row means: this role has these specific permissions on this screen."""

    __tablename__ = "Tagscan_role_permissions"

    # A role can only have ONE permissions row per screen.
    __table_args__ = (UniqueConstraint("role_id", "screen_id", name="uq_tagscan_role_permissions_role_screen"),)
    ...
    screen_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("Tagscan_screens.id", ondelete="CASCADE"), nullable=False
    )
