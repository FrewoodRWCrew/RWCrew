...
# the Intervention Requests module. To add a brand-new screen: add one
...
class ScreenDefinition:
    ...
    # The stable, code-friendly identifier, e.g. "interventionrequests.roles".
...
# Every screen Intervention Requests currently has: its three real screens
# ("Intervention Requests", "Intervention Statuses", "TeamKar"), and the two
# access-rights screens ("Roles", "Users"). All are gated independently of
# one another — a Requests permission does not imply a Statuses permission.
...
def sync_screens(db: Session) -> None:
    ...
    existing_screens_by_key = {
        screen.key: screen for screen in db.scalars(select(InterventionRequestsScreen)).all()
    }
    ...
    for definition in SCREEN_DEFINITIONS:
        ...
        if existing_screen is None:
            ...
            db.add(
                InterventionRequestsScreen(key=definition.key, label=definition.label, sort_order=definition.sort_order)
            )
        else:
