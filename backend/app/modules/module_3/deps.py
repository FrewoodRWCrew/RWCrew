...
# Intervention Requests endpoint. Intervention Requests doesn't use the
...
# admin/editor/reader role. See app/modules/module_9/deps.py for
# MasterData's identical copy of this same pattern.
...
MODULE_KEY = "module-3"
MODULE_LABEL = "Intervention Requests"
...
def require_module_access(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> User:
    ...
    Intervention Requests at all (super admins are not automatically
...
def get_user_role(db: Session, user_id: int) -> InterventionRequestsUserRole | None:
    """Look up the calling user's current Intervention Requests role
    assignment, if any.
    """
    return db.scalar(select(InterventionRequestsUserRole).where(InterventionRequestsUserRole.user_id == user_id))


def get_permission_for_screen(db: Session, role_id: int, screen_id: int) -> InterventionRequestsRolePermission | None:
    ...
    """
    return db.scalar(
        select(InterventionRequestsRolePermission).where(
            InterventionRequestsRolePermission.role_id == role_id,
            InterventionRequestsRolePermission.screen_id == screen_id,
        )
    )
...
def user_can(db: Session, user: User, screen_key: str, action: PermissionAction) -> bool:
    ...
    if user.is_super_admin:
        # The site-wide super admin can always do anything in Intervention
        # Requests — this is what lets them bootstrap the module's very
        # first role.
        return True

    screen = db.scalar(select(InterventionRequestsScreen).where(InterventionRequestsScreen.key == screen_key))
    if screen is None:
        return False
