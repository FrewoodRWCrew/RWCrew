...
# MasterData endpoint. MasterData doesn't use the shared, generic
# app/modules/common.py factory that most other modules use, because it
# needs a richer permission model: custom roles with a separate view/
# create/edit/delete switch per screen, instead of one fixed admin/editor/
# reader role. See app/modules/module_1/deps.py for TagScan's identical
# copy of this same pattern.
...
def require_module_access(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)) -> User:
    ...
    MasterData at all (super admins are not automatically exempt — matching
...
def get_user_role(db: Session, user_id: int) -> MasterDataUserRole | None:
    """Look up the calling user's current MasterData role assignment, if any."""
    return db.scalar(select(MasterDataUserRole).where(MasterDataUserRole.user_id == user_id))


def get_permission_for_screen(db: Session, role_id: int, screen_id: int) -> MasterDataRolePermission | None:
    ...
    """
    return db.scalar(
        select(MasterDataRolePermission).where(
            MasterDataRolePermission.role_id == role_id, MasterDataRolePermission.screen_id == screen_id
        )
    )
...
def user_can(db: Session, user: User, screen_key: str, action: PermissionAction) -> bool:
    ...
    if user.is_super_admin:
        # The site-wide super admin can always do anything in MasterData —
        # this is what lets them bootstrap the module's very first role.
        return True

    screen = db.scalar(select(MasterDataScreen).where(MasterDataScreen.key == screen_key))
    if screen is None:
        return False
