# Bulk XLSX import/export for the Product Limit lookup: building the
...
# only itself instead of aborting the whole batch. A row naming a limit
# option that already exists is REJECTED as an error rather than
...
from app.schemas.masterdata import ProductLimitImportRowResult

TEMPLATE_COLUMNS = ["name"]


def build_product_limit_template_xlsx() -> bytes:
    ...
    create a real limit option (import rejects duplicates rather than
    upserting, so it wouldn't even get caught as "already exists" the
    first time).
    """
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Limieten"
...
def import_product_limits_from_xlsx(db: Session, file_bytes: bytes) -> list[ProductLimitImportRowResult]:
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        try:
            ...
            existing = db.scalar(select(ProductLimit).where(ProductLimit.name == name))
            if existing is not None:
                raise ValueError("A limit option with this name already exists")

            db.add(ProductLimit(name=name))

            savepoint.commit()
            results.append(
                ProductLimitImportRowResult(row_number=row_number, name=name, outcome="created", detail=None)
            )
        except (ValueError, IntegrityError) as error:
            savepoint.rollback()
            detail = str(error) if isinstance(error, ValueError) else "This row conflicts with existing data"
            results.append(
                ProductLimitImportRowResult(row_number=row_number, name=name, outcome="error", detail=detail)
            )
...
def export_product_limits_to_xlsx(db: Session) -> bytes:
    """Every limit option as a single-sheet workbook."""
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Limieten"
    ...
    limits = db.scalars(select(ProductLimit).order_by(ProductLimit.name)).all()
    for limit in limits:
        sheet.append([limit.id, limit.name])
