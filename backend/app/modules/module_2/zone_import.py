# Bulk XLSX import/export for the Zone lookup: building the downloadable
# template, applying an uploaded workbook row by row, and exporting the
# full list back out. Mirrors kar_status_import.py's own shape (which does
# the same thing for the KarStatussen lookup), just for Zone.
#
...
# only itself instead of aborting the whole batch. A row naming a zone
# that already exists is REJECTED as an error rather than upserted — same
# rule as the Karren import, since a zone is just a name anyway.
...
from app.schemas.kartracker import ZoneImportRowResult

TEMPLATE_COLUMNS = ["name"]


def build_zone_template_xlsx() -> bytes:
    ...
    create a real zone (import rejects duplicates rather than upserting,
...
def import_zones_from_xlsx(db: Session, file_bytes: bytes) -> list[ZoneImportRowResult]:
    ...
    for row_number, row in enumerate(rows, start=2):
        ...
        try:
            ...
            existing = db.scalar(select(KarTrackerZone).where(KarTrackerZone.name == name))
            if existing is not None:
                raise ValueError("A zone with this name already exists")

            db.add(KarTrackerZone(name=name))

            savepoint.commit()
            results.append(ZoneImportRowResult(row_number=row_number, name=name, outcome="created", detail=None))
        except (ValueError, IntegrityError) as error:
            savepoint.rollback()
            detail = str(error) if isinstance(error, ValueError) else "This row conflicts with existing data"
            results.append(ZoneImportRowResult(row_number=row_number, name=name, outcome="error", detail=detail))

    db.commit()
    return results


def export_zones_to_xlsx(db: Session) -> bytes:
    """Every zone as a single-sheet workbook."""
    workbook = Workbook()
    sheet = workbook.active
    sheet.title = "Zone"
    ...
    zones = db.scalars(select(KarTrackerZone).order_by(KarTrackerZone.name)).all()
    for zone in zones:
        sheet.append([zone.id, zone.name])
